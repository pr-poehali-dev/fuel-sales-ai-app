      ...
      <div className="bg-[#111318] border border-[#1e2330] w-full md:max-w-md animate-scale-in rounded-t-2xl md:rounded-none">
        ...
        <div className="flex gap-3 px-5 pb-6 pt-1">
          <button onClick={() => setShowModal(false)}
            className="flex-1 py-3 border border-[#1e2330] text-sm text-[#8896aa] hover:border-[#8896aa] transition-colors">
            Отмена
          </button>
          <button onClick={() => setShowModal(false)}
            className="flex-1 py-3 bg-[#c9a84c] text-black text-sm font-semibold hover:bg-[#d4b85a] transition-colors">
            Отправить заявку
          </button>
        </div>
      </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="md:hidden h-14 border-b border-[#1e2330] bg-[#111318] flex items-center justify-between px-4 shrink-0">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-bold tracking-wider uppercase text-[#e2e8f0]">ТопливоХаб</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-emerald-400" />
            <span className="text-[11px] text-[#4a5568]">АО «АгроГрупп»</span>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-auto p-4 md:p-8 pb-24 md:pb-8">
          ...
          {section === "home" && (
            <div className="space-y-5 md:space-y-7 animate-fade-in">
              <div>
                <h1 className="text-lg md:text-xl font-semibold text-[#e2e8f0] tracking-tight">Добро пожаловать, АО «АгроГрупп»</h1>
                <p className="text-xs text-[#4a5568] mt-1">Данные по закупкам топлива на 06.05.2026</p>
              </div>

              {/* KPI — 2x2 on mobile, 4x1 on desktop */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                {[
                  ...
                  { label: "Цена ДТ", value: "64,20 ₽/л", icon: "Fuel", sub: "+1,10 ₽ за неделю" },
                ].map((kpi) => (
              ...
              <button onClick={() => openOrder()}
                className="w-full bg-[#c9a84c] text-black p-4 flex items-center gap-3 hover:bg-[#d4b85a] transition-colors text-left">
                <Icon name="Plus" size={18} />
                <div>
                  <div className="text-sm font-semibold">Создать заявку</div>
                  <div className="text-[11px] opacity-70 mt-0.5">ИИ заполнит по истории заказов</div>
                </div>
              </button>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setSection("catalog")}
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-[#4a5568] mt-0.5">6 позиций</div>
                  </div>
                </button>
                <button onClick={() => setSection("orders")}
                  className="bg-[#111318] border border-[#1e2330] p-4 flex items-center gap-3 hover:bg-[#1a1f2e] transition-colors text-left">
                  <Icon name="Truck" size={16} className="text-[#8896aa] shrink-0" />
                  <div>
                    <div className="text-sm font-semibold text-[#e2e8f0]">Заявки</div>
                    <div className="text-[10px] text-[#4a5568] mt-0.5">3 активных</div>
                  </div>
                </button>
              </div>
                ...
                {ORDERS.map((o) => (
                  <div key={o.id} className="bg-[#111318] border border-[#1e2330] p-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      {[
                        { label: "Объём", value: o.volume },
                        { label: "Сумма", value: o.total },
                        { label: "Дата", value: o.date },
                      ].map((f) => (
                        <div key={f.label}>
            ...
            <div className="space-y-5 animate-fade-in">
              ...
              <div className="bg-[#111318] border border-[#1e2330] p-5">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  {[
                    { label: "Предпочт. топливо", value: "ДТ Летнее / Зимнее" },
                    { label: "Стандартный объём", value: "5 000 — 10 000 л" },
                    { label: "Адрес доставки", value: "Москва, Промышленная, 12" },
                    { label: "Время приёма", value: "08:00 — 18:00" },
                    { label: "Контактное лицо", value: "Иванов А.В." },
          ...
          {section === "contacts" && (
            <div className="space-y-5 animate-fade-in">
              <div>
                <h1 className="text-lg md:text-xl font-semibold text-[#e2e8f0] tracking-tight">Контакты и поддержка</h1>
              ...
              <div className="bg-[#111318] border border-[#1e2330] p-5">
                <div className="text-[9px] uppercase tracking-[0.2em] text-[#4a5568] mb-4">Офис компании</div>
          ...
          {navItems.map((item) => (
            <button key={item.id} onClick={() => setSection(item.id)}
              className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 transition-colors ${
                section === item.id ? "text-[#c9a84c]" : "text-[#4a5568]"
              }`}>
